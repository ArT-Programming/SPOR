...
public:
	...
	void getPixels(unsigned highlights, unsigned shadows, Vec3f winSize, int resX, int resY, Vec3f scale, Vec3f pos, Texture mask){
		...
		//texture[frame].format(Graphics::BGR);
		...
		//unsigned char * texBufMask = mask.data<unsigned char>();
		...
		for(int j=0; j<Ny; j++){
			for(int i=0; i<Nx; i++){
				...
				int pixX = (((coord.x + (winSize.x / 2.)) / winSize.x) * resX);// + (resX / 2.);
				int pixY = (((coord.y + (winSize.y / 2.)) / winSize.y) * resY);// + (resY / 2.);
				
				//int pixX = resX * (pos.x + (winSize.x * scale.x * i / float(Nx))) / winSize.x;
				//int pixY = resY * (pos.y + (winSize.y * scale.y * j / float(Ny))) / winSize.y;
				//std::cout<<Vec2i(i,j)<<"  "<<Vec2f(coord.x,coord.y)<<"  "<<Vec2f(winSize.x,winSize.y)<<"  "<<Vec2i(pixX,pixY)<<"  "<<Vec2i(resX,resY)<<"\n \n";
					
				
				if(pixX >= 0 && pixY >= 0 && pixX < resX && pixY < resY){
				*/
				...
				<<Vec2i(pixX,pixY)<<"  "
				<<Vec2f(winSize.x,winSize.y)<<"  "
				<<Vec2i(resX,resY)<<"\n \n";*/
				
				/*
					unsigned long idxMask = pixY * resX + pixX;
					int strideMask = mask.numComponents();
					//std::cout<<Vec2f(pixX,pixY)<<"   "<<idxMask*strideMask<<"   "<<int(texBufMask[1+idxMask*strideMask])<<"\n";
					if(texBufMask[idxMask*strideMask] > 127){
